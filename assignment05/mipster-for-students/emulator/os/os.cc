...
DEFINE_bool(load_raw, false, "Load raw instruction files instead of ELF");
...
void OS::Init() {
  ...
  for (auto it = inputs_.begin(); it != inputs_.end(); ++it) {
    ...
    uint32_t offset;
    uint32_t end_address; // this is where the heap will start
    uint32_t entry_point;
    if (FLAGS_load_raw) {
      entry_point = loader.LoadRaw(
        program_name, Mips32Machine::Get().MachineState().physical_memory,
        &offset, &end_address);
    } else {
      entry_point = loader.LoadElf_32LE(
        program_name, Mips32Machine::Get().MachineState().physical_memory,
        &offset, &end_address);
    }
    ms.registers.sp = p->stack_addr();
    ms.pc = entry_point;
    ms.registers.k1 = end_address;
    p->SaveContext(ms);
...
void OS::ExceptionHandler(ExceptionType type) {
  ...
  switch (type) {
    ...
    case kReservedInstruction:
      ...
      break;
    case kSignalException:
      errx(EXIT_FAILURE, "recived signal exception");
      break;
    default:
...
void OS::Syscall() {
  ...
  switch (syscall_nr) {
    ...
    case kMipsSyscallElfSize:
      SyscallElfSize();
      break;
    case kMipsSyscallBumpPointer:
      SyscallBumpPointer();
      break;
    case kMipsSyscallGetchar:
      SyscallGetchar();
      break;
    case kMipsSyscallCreateProcess:
      SyscallCreateProcess();
      break;
    case kMipsSyscallDumpRaw:
      SyscallDumpRaw();
      break;
    default:
