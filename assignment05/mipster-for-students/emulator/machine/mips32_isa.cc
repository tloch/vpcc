...
void TrapIfEqualInstruction::Execute() {
  Disassemble("teq %s, %s",
              Register::Name(InstructionData().rs),
              Register::Name(InstructionData().rt));
  ...
  if (rs == rt) {
    OS::Get().ExceptionHandler(ExceptionType::kSignalException);
    UNREACHABLE();
...
void MultiplyWordRegisterInstruction::Execute() {
  // Registers HI and LO are unpredictable, just leave them. 
  ...
  WriteRegister(InstructionData().rd, static_cast<uint32_t>(result));
...
//  ErrorOut(EXIT_FAILURE, ISAM, "slt not yet implemented");
  Disassemble("slt %s,%s,%s",
              ...
              Register::Name(InstructionData().rs),
              Register::Name(InstructionData().rt));
/* reference for handling signed register contents, copied from "add":
    int64_t temp =
      static_cast<int64_t>(MachineState().registers.GPR[InstructionData().rs]) +
      static_cast<int64_t>(MachineState().registers.GPR[InstructionData().rt]);*/
// rest copied from SLTU
